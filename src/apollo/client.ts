...
const operationEndpoint = ApolloLink.from([
  ...
  createSubscriptionHandshakeLink({ url: linkOpr, region, auth: authOpr }, httpLinkOpr),
])
...
const expoEndpoint = ApolloLink.from([
  ...
  createSubscriptionHandshakeLink({ url: linkExpo, region, auth: authExpo }, httpLinkExpo),
])
